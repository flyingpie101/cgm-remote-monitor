  ...
  loop.sendNotification = function sendNotification (data, remoteAddress, completion) {
    ...
    if (data.eventType === 'Temporary Override Cancel') {
      ...
      if(payload["carbs-entry"] > 0.0 ) {
        ...
        if (data.meal-type !== undefined) { //Auggie - get info on the type of meal being eaten
          payload['meal-type'] = data.mealType; //Auggie - get info on the type of meal being eaten
        } //Auggie - get info on the type of meal being eaten
         alert  = "Remote Carbs Entry: "+payload["carbs-entry"]+" grams\n";
         //alert  = "Meal type: "+payload["meal-type"]; //Auggie - show the type of meal (what is being eaten)
         alert += "Absorption Time: "+payload["absorption-time"]+" hours";
         //alert += "Meal Time: "+payload["start-time"]; //Auggie - show the time of meal (what time eating begins)
      } else {
